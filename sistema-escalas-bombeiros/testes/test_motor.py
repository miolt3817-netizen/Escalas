...
def test_indice_de_ausencias_bate_com_a_busca_linear():
    """O índice substituiu a varredura por (bombeiro, dia). Precisa dar igual.

    A varredura custava milhões de comparações com histórico de dois anos;
    o índice resolve em tempo constante, mas só serve se a resposta for a mesma.
    """
    from motor.equidade import esta_disponivel, indice_de_ausencias

    bombeiros = [Bombeiro(i, f"B{i}") for i in range(1, 8)]
    ausencias = [
        Indisponibilidade(1, date(2027, 3, 5), date(2027, 3, 12),
                          TipoIndisponibilidade.FERIAS),
        Indisponibilidade(3, date(2027, 3, 1), date(2027, 3, 1),
                          TipoIndisponibilidade.ATESTADO),
        Indisponibilidade(3, date(2027, 3, 20), date(2027, 4, 10),
                          TipoIndisponibilidade.LICENCA),
        Indisponibilidade(5, date(2027, 2, 25), date(2027, 3, 3),
                          TipoIndisponibilidade.AFASTAMENTO),
    ]
    dias = dias_do_mes(2027, 3)
    indice = indice_de_ausencias(ausencias, dias)

    for b in bombeiros:
        for d in dias:
            assert ((b.id, d) in indice) == (not esta_disponivel(b.id, d, ausencias)), (
                f"índice divergiu para bombeiro {b.id} em {d}"
            )


def test_elegibilidade_nao_muda_com_o_indice():
    """Blindagem do cálculo de equidade contra a otimização."""
    from motor.calendario import dias_por_categoria, mapa_categorias
    from motor.equidade import dias_elegiveis

    bombeiros = [Bombeiro(i, f"B{i}") for i in range(1, 6)]
    ausencias = [
        Indisponibilidade(2, date(2027, 5, 10), date(2027, 5, 20),
                          TipoIndisponibilidade.FERIAS),
    ]
    por_cat = dias_por_categoria(
        mapa_categorias(dias_do_mes(2027, 5), [Feriado(date(2027, 5, 1))])
    )
    elegiveis = dias_elegiveis(bombeiros, por_cat, ausencias)

    # quem não faltou tem todos os dias da categoria
    assert elegiveis[1][Categoria.TOTAL] == 31
    # quem faltou 11 dias tem 11 a menos no total
    assert elegiveis[2][Categoria.TOTAL] == 31 - 11
    # e a soma por categoria nunca passa do total de dias daquela categoria
    for b in bombeiros:
        for cat, dias in por_cat.items():
            assert 0 <= elegiveis[b.id][cat] <= len(dias)
