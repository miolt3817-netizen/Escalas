...
with sync_playwright() as pw:
    ...
    page.click('[data-vista="config"]')
    page.wait_for_timeout(900)
    page.click('#vista-config .sub-aba[data-sub="equipe"]')
    page.wait_for_timeout(1200)
    ...
    checar(not pb.is_visible('[data-vista="config"]'),
           "aba Configurações some para bombeiro")
    checar(not pb.is_visible("#btn-gerar"), "botão Gerar some")
    checar(not pb.is_visible("#btn-publicar"), "botão Publicar some")
    # A aba é "Datas" para todos; quem muda por papel é o título da página.
    pb.click('[data-vista="datas"]')
    pb.wait_for_timeout(1200)
    checar("Minhas datas" in pb.inner_text("#titulo-datas"),
           f"título da página: {pb.inner_text('#titulo-datas')}")
    pb.click("#btn-ajuda")
    ...
    checar(pb.is_visible("#lista-trocas"),
           "trocas aparecem na mesma página, abaixo das datas")
