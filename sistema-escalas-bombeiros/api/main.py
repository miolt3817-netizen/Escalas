...
app = FastAPI(
    ...
    title="Escalas BM — sistema de escalas para bombeiros",
    description=(
...
class UsuarioEntrada(BaseModel):
    ...
    #: Como a pessoa é chamada no quartel. Vazio deriva do nome completo.
    nome_curto: str = Field(default="", max_length=40)
    telefone: str = Field(default="", max_length=25)
    email: str = Field(min_length=5, max_length=160)
    #: Como a pessoa é chamada no quartel. Vazio: derivado do nome completo.
    nome_curto: str = Field(default="", max_length=40)
...
class UsuarioEdicao(BaseModel):
    ...
    nome_curto: str | None = Field(default=None, max_length=40)
...
def criar_usuario(
    ...
    usuario = m.Usuario(
        ...
        nome_curto=(dados.nome_curto.strip() or s.derivar_nome_curto(dados.nome)),
...
def editar_usuario(
    ...
    if dados.nome_curto is not None:
        alvo.nome_curto = dados.nome_curto.strip() or s.derivar_nome_curto(alvo.nome)
    if dados.telefone is not None:
        alvo.telefone = dados.telefone.strip()
    if dados.papel:
...
def obter_escala(
    ...
    pessoas = {u.id: u for u in db.scalars(select(m.Usuario)).all()}
    nomes = {i: u.nome for i, u in pessoas.items()}
    feriados = {
        f.data: f.nome
        ...
        "plantoes": [
            {
                ...
                "bombeiro_curto": (
                    pessoas[p.bombeiro_id].nome_curto or nomes.get(p.bombeiro_id, "?")
                    if p.bombeiro_id in pessoas else "?"
                ),
...
def explicar_dia(
    ...
    # Valida a data ANTES de tocar o banco: `date(2026, 9, 99)` estoura, e o
    # cliente recebia 500 no lugar de um erro claro.
    try:
        alvo = date(ano, mes, dia)
    except ValueError:
        raise HTTPException(status_code=422, detail="Data inválida.") from None
...
def substitutos(
    ...
    # Valida a data primeiro: o solver leva segundos, e não faz sentido rodá-lo
    # para depois estourar montando uma data que não existe.
    try:
        alvo = date(ano, mes, dia)
    except ValueError:
        raise HTTPException(status_code=422, detail="Data inválida.") from None
    ...
    return {
        "data": alvo.isoformat(),
        "sugestoes": sugerir_substitutos(entrada, resultado, alvo),
    }
...
def porque_esta_escala(
    ...
    # É a resposta à pergunta que aparece em toda reunião: "por que eu peguei
    # três domingos e o Fulano nenhum?". Sem isto o supervisor tinha o número
    # só no relatório anual, em outra tela.
    def _conta(bombeiro_id: int, filtro) -> int:
        return sum(1 for d, bid in escalados.items() if bid == bombeiro_id and filtro(d))

    distribuicao = sorted(
        (
            {
                "bombeiro": nomes.get(b.id, "?"),
                "total": _conta(b.id, lambda d: True),
                "vermelha": _conta(b.id, lambda d: d.weekday() >= 5 or d in feriados),
                "sabado": _conta(b.id, lambda d: d.weekday() == 5),
                "domingo": _conta(b.id, lambda d: d.weekday() == 6),
                "feriado": _conta(b.id, lambda d: d in feriados),
            }
            for b in ativos
        ),
        key=lambda linha: (-linha["vermelha"], -linha["total"], linha["bombeiro"]),
    )
    ...
    # As indisponibilidades são carregadas UMA vez e cruzadas em memória. A
    # versão anterior consultava o banco por dia e por bombeiro: com 31 dias e
    # 39 pessoas, eram 1.209 consultas para montar um painel.
    todas_ausencias = db.scalars(select(m.Indisponibilidade)).all()
    ausente_em: dict[date, set[int]] = {d: set() for d in dias}
    for indisp in todas_ausencias:
        for d in dias:
            if indisp.inicio <= d <= indisp.fim:
                ausente_em[d].add(indisp.bombeiro_id)

    apertados = []
    for d in dias:
        disponiveis = [b for b in ativos if b.id not in ausente_em[d]]
...
@app.get("/bombeiros/{bombeiro_id}/extrato", tags=["inteligencia"])
def extrato_do_bombeiro(
    bombeiro_id: int,
    ano: int,
    db: Session = Depends(obter_sessao),
    usuario: m.Usuario = Depends(usuario_atual),
):
    """A resposta para "por que peguei três domingos?".

    A pergunta é sempre comparativa — "três, e o colega nenhum" —, então o
    número sozinho não resolve. Aqui vai o que a pessoa pegou, quanto seria a
    parcela justa dela, e como ela se compara à média da equipe, categoria por
    categoria, com os meses discriminados.
    """
    alvo = db.get(m.Usuario, bombeiro_id)
    if alvo is None:
        raise HTTPException(status_code=404, detail="Bombeiro não encontrado.")
    if usuario.papel == BOMBEIRO and usuario.id != bombeiro_id:
        raise HTTPException(
            status_code=403, detail="Você só consulta o próprio extrato."
        )
    ...
    def classificar(plantao: m.Plantao) -> list[str]:
        cats = ["total", plantao.tipo]
        if plantao.data.weekday() == 5:
            cats.append("sabado")
        if plantao.data.weekday() == 6:
            cats.append("domingo")
        if plantao.data in feriados:
            cats.append("feriado")
        return cats

    CATEGORIAS = ["total", "branca", "vermelha", "sabado", "domingo", "feriado"]
    meu = {c: 0 for c in CATEGORIAS}
    equipe: dict[int, dict[str, int]] = {b.id: {c: 0 for c in CATEGORIAS} for b in ativos}
    por_mes: dict[int, dict[str, int]] = {}

    for escala in escalas:
        for p in escala.plantoes:
            cats = classificar(p)
            if p.bombeiro_id in equipe:
                for c in cats:
                    equipe[p.bombeiro_id][c] += 1
            if p.bombeiro_id == bombeiro_id:
                for c in cats:
                    meu[c] += 1
                linha = por_mes.setdefault(
                    escala.mes, {c: 0 for c in CATEGORIAS}
                )
                for c in cats:
                    linha[c] += 1

    saldos = s.saldo_historico(db, ano + 1, 1)
    meu_saldo = saldos.get(bombeiro_id, {})

    comparativo = []
    for c in CATEGORIAS:
        valores = [v[c] for v in equipe.values()]
        media = sum(valores) / len(valores) if valores else 0
        comparativo.append(
            {
                "categoria": c,
                "meus": meu[c],
                "media_da_equipe": round(media, 1),
                "menor": min(valores) if valores else 0,
                "maior": max(valores) if valores else 0,
                "saldo": round(meu_saldo.get(Categoria(c), 0.0), 2),
            }
        )
    ...
    return {
        ...
        "nome": alvo.nome,
        "ano": ano,
        "meses_publicados": sorted(por_mes),
        "comparativo": comparativo,
        "por_mes": [
            {"mes": mes, **valores} for mes, valores in sorted(por_mes.items())
        ],
        "equipe_ativa": len(ativos),
    }
