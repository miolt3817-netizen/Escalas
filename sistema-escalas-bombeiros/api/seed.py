...
#: (nome, usuário, nome curto, telefone, ausências, preferências, ativo)
#:
#: O nome curto é como a pessoa é chamada no quartel — ninguém diz o nome
#: completo, e ele não cabe na célula do calendário. O telefone existe para o
#: supervisor ligar de dentro do sistema quando precisa de substituto.
#: Ausência:    (início, fim, tipo)
#: Preferência: (tipo, data, dia_semana, peso) — 0 = segunda ... 6 = domingo
#:
...
EQUIPE = [
    (
        "Anderson Duarte Prates", "anderson.prates", "Sd. Prates", "(49) 99101-2233",
        [
    ...
    (
        "Cristiane Balbinot Sfredo", "cristiane.sfredo", "Cb. Sfredo", "(49) 99102-3344",
        [(date(2026, 3, 2), date(2026, 3, 31), "ferias")],
        [("quer", None, 5, 1), ("evita", None, 2, 1)],  # sábado rende; quarta é dia do filho
        True,
    ),
    (
        "Everton Kaufmann Reis", "everton.reis", "Sd. Reis", "(49) 99103-4455",
        [
    ...
    (
        "Gabriela Nunes Tavares", "gabriela.tavares", "Cb. Tavares", "(49) 99104-5566",
        [(date(2026, 7, 6), date(2026, 8, 4), "ferias")],
        [("evita", None, 4, 2), ("evita", None, 5, 2)],  # mora em outra cidade, viaja no fim de semana
        True,
    ),
    (
        "Henrique Bortolini Vaz", "henrique.vaz", "Sd. Vaz", "(49) 99105-6677",
        [
    ...
    (
        "Juliana Weber Colombo", "juliana.colombo", "Sgt. Colombo", "(49) 99106-7788",
        [
    ...
    (
        "Leandro Piccoli Machado", "leandro.machado", "Sd. Machado", "(49) 99107-8899",
        [(date(2026, 6, 8), date(2026, 7, 7), "ferias")],
        [],                               # sem preferência declarada
        True,
    ),
    (
        "Michele Grando Sartori", "michele.sartori", "Cb. Sartori", "(49) 99108-9900",
        [
    ...
    (
        "Rodrigo Bavaresco Milani", "rodrigo.milani", "Sd. Milani", "(49) 99109-0011",
        [(date(2026, 4, 6), date(2026, 5, 5), "ferias")],
        [("evita", None, 1, 1), ("evita", None, 3, 1)],  # treina para concurso
        True,
    ),
    (
        "Simone Pagnussat Oliveira", "simone.oliveira", "Ten. Oliveira", "(49) 99110-1122",
        [
    ...
    (
        "Vilmar Chiapinotto Reck", "vilmar.reck", "Sgt. Reck", "",
        [],
        [],
...
def _semear_usuarios(db) -> list[tuple[str, str, str]]:
    ...
    contas = [
        ...
        ("Sgt. Roberto Nascimento", f"supervisor@{DOMINIO}", "supervisor",
         "Sgt. Nascimento", "(49) 99100-1122"),
    ] + [
        (nome, f"{usuario}@{DOMINIO}", "bombeiro", curto, tel)
        for nome, usuario, curto, tel, _aus, _pref, _ativo in EQUIPE
    ]

    for nome, email, papel, curto, telefone in contas:
...
def _preencher_nomes_curtos(db) -> None:
    """Cadastro anterior à coluna fica sem nome curto; deriva do nome completo.

    Sem isto, quem atualizou o sistema veria o calendário voltar a mostrar
    nome completo até editar pessoa por pessoa.
    """
    from .servicos import derivar_nome_curto

    faltando = [
        u for u in db.scalars(select(m.Usuario)).all() if not u.nome_curto
    ]
    for u in faltando:
        u.nome_curto = derivar_nome_curto(u.nome)
    if faltando:
        db.commit()
        print(f"[seed] {len(faltando)} nome(s) curto(s) derivado(s) do nome completo.")
...
def _semear_ausencias_e_preferencias(db) -> None:
    ...
    for nome, usuario, _curto, _tel, ausencias, preferencias, ativo in EQUIPE:
        pessoa = db.scalars(
