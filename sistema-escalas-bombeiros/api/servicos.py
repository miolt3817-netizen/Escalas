...
def derivar_nome_curto(nome: str) -> str:
    """Primeiro nome + último sobrenome: "Cristiane Sfredo".

    No quartel ninguém usa o nome completo, e ele não cabe na célula do
    calendário. Quando o cadastro não traz como a pessoa é chamada, esta é a
    aproximação menos ruim.
    """
    partes = [p for p in (nome or "").split() if len(p) > 2]
    if len(partes) <= 1:
        return (nome or "").strip()[:40]
    return f"{partes[0]} {partes[-1]}"[:40]
...
def apelido(usuario: m.Usuario) -> str:
    """Como a pessoa é chamada no quartel.

    Usa o `nome_curto` quando existe. Sem ele, monta "Primeiro Último" — nome
    completo ocupa três linhas na célula do calendário, e o supervisor precisa
    bater o olho, não ler.
    """
    if usuario.nome_curto.strip():
        return usuario.nome_curto.strip()
...
def candidatos_para(db: Session, escala: m.Escala, alvo: date) -> list[dict]:
    ...
    entrada = montar_entrada(db, escala.ano, escala.mes, preservar_travados=False)
    # `montar_entrada` já calculou o saldo histórico; recalcular dobrava o
    # trabalho mais caro da função.
    saldos = entrada.saldo_historico
    pessoas = {u.id: u for u in db.scalars(select(m.Usuario)).all()}
    nomes = {i: u.nome for i, u in pessoas.items()}
    # O supervisor precisa LIGAR para o substituto; sem o telefone aqui ele
    # sai do sistema para procurar o contato no celular.
    telefones = {i: u.telefone for i, u in pessoas.items()}

    preferencias = db.scalars(select(m.Preferencia)).all()
    indisponibilidades = db.scalars(select(m.Indisponibilidade)).all()

    # Trocar quem cobre UM dia só pode violar regra naquele dia ou nos vizinhos
    # imediatos. Rodar o verificador do mês inteiro para cada candidato custava
    # meio segundo com 39 bombeiros; aqui a checagem é local.
    folga = entrada.parametros.intervalo_minimo_dias
    janela = [alvo + timedelta(days=k) for k in range(-folga, folga + 1) if k]
    anteriores = {(pa.bombeiro_id, pa.data) for pa in entrada.plantoes_anteriores}

    def impedimento(bombeiro_id: int) -> str:
        """Motivo pelo qual esta pessoa não pode assumir o dia, ou vazio."""
        ausencia = next(
            (
                i for i in indisponibilidades
                if i.bombeiro_id == bombeiro_id and i.inicio <= alvo <= i.fim
            ),
            None,
        )
        if ausencia:
            artigo = "de" if ausencia.tipo == "ferias" else "em"
            return f"está {artigo} {ausencia.tipo} nesta data"

        for vizinho in janela:
            trabalha = (
                atual.get(vizinho) == bombeiro_id
                or (bombeiro_id, vizinho) in anteriores
            )
            if trabalha:
                return f"trabalha em {vizinho:%d/%m}, sem o descanso mínimo"
        return ""
    ...
    for bombeiro in bombeiros_ativos(db):
        ...
        bloqueio = impedimento(bombeiro.id)
        elegivel = not bloqueio
        ...
        saida.append(
            {
                ...
                # O supervisor precisa LIGAR para quem vai cobrir. Sem isto ele
                # sai da tela para procurar o contato no celular.
                "telefone": telefones.get(bombeiro.id, ""),
                "telefone": telefones.get(bombeiro.id, ""),
                "elegivel": elegivel,
                "bloqueio": bloqueio,
                "nota": nota,
