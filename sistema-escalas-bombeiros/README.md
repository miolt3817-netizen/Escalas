# Escalas BM — Sistema de escalas para bombeiros
...
Marca **Escalas BM**: capacete de bombeiro sobre calendário, com confirmação.
...
**Ao trocar a marca, renomeie os arquivos de ícone.** O navegador guarda o
ícone de aplicativo instalado com muita persistência: alterar o conteúdo
mantendo o nome não força a atualização, e o usuário continua vendo o símbolo
antigo na área de trabalho. Daí o sufixo `-v2` nos arquivos atuais.
...
testes/           118 testes + 2 scripts de caçada a bugs
```
...
A navegação é por **barra superior**, com quatro abas:

| Aba | Reúne | Quem vê |
|---|---|---|
| **Escala** | calendário, tabela, exportação | todos |
| **Datas** | ausências e preferências, com as trocas empilhadas abaixo | todos |
| **Configurações** | Equipe e Geração de escala, em sub-abas | supervisor e admin |
| **Relatórios** | números do ano e registro de alterações | supervisor e admin |

Quatro abas cabem na barra sem disputar espaço com os botões de ação — o
problema que existia com seis. Em telas estreitas a barra rola na horizontal,
sem esconder seção atrás de menu.

Datas e trocas ficaram **empilhadas na mesma rolagem**, não em sub-abas: uma
troca depende das datas informadas acima, e separá-las obrigaria a ir e voltar.
Já Equipe e Geração de escala não se relacionam, então mantêm sub-abas.

O dia corrente é marcado em **azul**, não em vermelho: o vermelho já significa
marca e escala de fim de semana, e um terceiro sentido o tornaria ilegível.

### Gerar escala

Tela própria com os **parâmetros do motor visíveis** — folga obrigatória, folga
desejável e duração do plantão. Eles existiam apenas na API: o supervisor não
tinha como ver nem ajustar a decisão de domínio mais sensível do sistema.

Ao lado, as regras que sempre valem e a ordem dos seis critérios, para conferir
o que está em vigor. E o resultado da última geração em números.
...
Clicar no nome de alguém — no painel de saldo ou na tabela de Relatórios —
abre o extrato do ano: plantões por categoria, **média da equipe** e o
intervalo entre quem pegou menos e quem pegou mais, mais o detalhamento mês a
mês.

Existe por causa de uma pergunta concreta: *"por que peguei três domingos e o
colega nenhum?"*. Ela é sempre comparativa, e o número isolado não responde. Os
dados já estavam no sistema, mas espalhados entre duas telas e agregados por
ano — o supervisor não tinha como responder na hora, na reunião.

O bombeiro consulta apenas o próprio extrato: a distribuição de plantões de um
colega é assunto do supervisor.
...
### O que veio de usar o sistema como escalante

Um percurso completo na pele de quem monta escala revelou seis lacunas que
nenhuma leitura de código pegaria:

| Faltava | Agora |
|---|---|
| Nada marcava o dia corrente | Dia de hoje destacado com etiqueta — é a primeira pergunta da manhã |
| Nome completo não cabia na célula | **Nome curto** ("Sd. Prates"), como se fala no quartel; o completo fica no `title` |
| Sem telefone de ninguém | Telefone no cadastro, exibido junto do candidato, com botão **Ligar** |
| Atestado custava 3 telas e 8 cliques | Atalho **"Faltou hoje?"** dentro do painel do dia |
| Sem como mandar no grupo | **Copiar texto** formatado para WhatsApp, com fins de semana marcados |
| "Por que peguei três domingos?" sem resposta direta | Tabela de distribuição **do mês** no painel "Por quê?" |

O telefone foi o achado mais revelador: o sistema ordenava seis substitutos por
saldo de equidade — o trabalho difícil — e parava exatamente antes do fácil,
deixando o supervisor procurar o contato no celular.
